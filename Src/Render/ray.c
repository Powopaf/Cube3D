...
/*   Updated: 2026/05/10 21:44:32 by sbrochar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	move_ray(double info[4], int map_xy[2], t_map map, t_p p)
{
	...
	if (map_xy[0] < 0 || map_xy[0] >= map.map_width || map_xy[1] < 0
		|| map_xy[1] >= map.map_height)
		return (INFINITY);
...
static void	ray_dist_info(double info[4], t_p p, double ray_angle)
{
...
double	ray_dist(t_p p, double ray_angle, t_map map, t_wall *wall)
{
	...
	ray_dist_info(info, p, ray_angle);
	prev_map[0] = (int)(info[2] / map.tile_size);
	prev_map[1] = (int)(info[3] / map.tile_size);
	while (1)
	{
		...
		if (dist >= 0.0)
		{
			find_face(&wall->face, map_xy, info, prev_map);
			wall->wall_x = info[2];
			wall->wall_y = info[3];
			return (dist);
		}
